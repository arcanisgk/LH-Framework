...
namespace Repository\Default\Shortcut\Back;
 ...
 * Class that handles:
 *
 * @package Repository\Default\Shortcut\Back;
 */
class Main extends FrontResource implements ControllerInterface
{
     ...
     * @throws Exception
     */
    private function getShortcut(): array
    {

        $default_resource_dic = implode(DS, [PD, 'Asset', 'resource', 'dic', 'shortcut.json']);
        $default_resource     = implode(DS, [PD, 'Asset', 'resource', 'shortcut', 'default.json']);

        $shortcutsJson = $this->render
            ->setPath($default_resource)
            ->setDic($default_resource_dic)
            ->render();
