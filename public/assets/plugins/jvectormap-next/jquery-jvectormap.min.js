...
Array.prototype.indexOf || (Array.prototype.indexOf = function (searchElement, fromIndex) {
    ...
    "object" == typeof exports ? module.exports = factory : "function" == typeof define && define.amd ? define(["jquery"], factory) : factory(jQuery)
}(function ($) {
    jvm.$ = $;
    var apiParams = {
    ...
    var special = $.event.special.mousewheel = {
        version: "3.1.9", setup: function () {
            ...
            if (this.addEventListener) for (var i = toBind.length; i;) this.addEventListener(toBind[--i], handler, ['wheel', 'mousewheel'].includes(toBind[i]) ? { passive: true } : false); else this.onmousewheel = handler;
    ...
    if (this.width = width, this.height = height, this.node.style.width = width + "px", this.node.style.height = height + "px", this.node.coordsize = width + " " + height, this.node.coordorigin = "0 0", this.rootElement) {
        for (paths = this.rootElement.node.getElementsByTagName("shape"), i = 0, l = paths.length; i < l; i++) paths[i].coordsize = width + " " + height, paths[i].style.width = width + "px", paths[i].style.height = height + "px";
        for (groups = this.node.getElementsByTagName("group"), i = 0, l = groups.length; i < l; i++) groups[i].coordsize = width + " " + height, groups[i].style.width = width + "px", groups[i].style.height = height + "px"
    }
    ...
    return path = path.replace(/(-?\d+)e(-?\d+)/g, "0"), path.replace(/([MmLlHhVvCcSs])\s*((?:-?\d*(?:\.\d+)?\s*,?\s*)+)/g, function (segment, letter, coords, index) {
    ...
    setAttributes: function (key, attr) {
        ...
        if (this.scale instanceof jvm.OrdinalScale || this.scale instanceof jvm.SimpleScale) for (cc in values) values[cc] ? attrs[cc] = this.scale.getValue(values[cc]) : attrs[cc] = this.elements[cc].element.style.initial[this.params.attribute]; else {
            if (void 0 === this.params.min || void 0 === this.params.max) for (cc in values) val = parseFloat(values[cc]), val > max && (max = val), val < min && (min = val);
            void 0 === this.params.min ? (this.scale.setMin(min), this.params.min = min) : this.scale.setMin(this.params.min), void 0 === this.params.max ? (this.scale.setMax(max), this.params.max = max) : this.scale.setMax(this.params.max);
            for (cc in values) "indexOf" != cc && (val = parseFloat(values[cc]), isNaN(val) ? attrs[cc] = this.elements[cc].element.style.initial[this.params.attribute] : attrs[cc] = this.scale.getValue(val))
        }
...
jvm.MapObject.prototype.setStyle = function () {
    ...
    this.config = config, this.map = this.config.map, wrapper = config.canvas.addGroup(config.canvas.rootElement), this.shape = config.canvas.addPath({d: config.path}, config.style, wrapper), config.canvas.addPath({
        d: config.path,
    ...
    }, wrapper).addClass("jvectormap-region jvectormap-element"), bbox = this.shape.getBBox(), text = this.getLabelText(config.code), this.config.label && text && (offsets = this.getLabelOffsets(config.code), this.labelX = bbox.x + bbox.width / 2 + offsets[0], this.labelY = bbox.y + bbox.height / 2 + offsets[1], this.label = config.canvas.addText({
        text: text,
        "text-anchor": "middle",
    ...
    createTip: function () {
        var map = this;
        jvm.$("body > .jvectormap-tip").remove(), this.tip = jvm.$("<div/>").addClass("jvectormap-tip").appendTo(jvm.$("body")), this.container.mousemove(function (e) {
    ...
    setFocus: function (config) {
        ...
        if (config = config || {}, config.region ? codes = [config.region] : config.regions && (codes = config.regions), codes) {
            for (i = 0; i < codes.length; i++) this.regions[codes[i]] && (itemBbox = this.regions[codes[i]].element.shape.getBBox()) && (void 0 === bbox ? bbox = itemBbox : (newBbox = {
